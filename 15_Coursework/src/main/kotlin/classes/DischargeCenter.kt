...
class DischargeCenter(numDischargePort: Int, wareHouse: Warehouse) {
    ...
    fun loadWaitTruck(tr: Truck) {
        waitTruck.add(tr)
        println("Грузовик с номером ${tr.numberTrack} поставлен в очередь на разгрузку")
    }
...
class DischargePort(wareHouse: Warehouse) {
    ...
    suspend fun dischargeTruck(tr: Truck) {
        ...
        println("Грузовик с номером ${tr.numberTrack} поставлен в порт разгрузки")
        isEmpty = false
        ...
        println("Разгружена машина с номером ${tr.numberTrack} и весом $dischargeWeight")
        isEmpty = true
    }
}